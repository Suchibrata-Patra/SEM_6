...
x.hat.hat = 38.4375
weight_x.hat = c(30,35,47,50,38,36,42,48,x.hat,32,41,52,40,34,38,54,40,35,34,45,40,34,38,42,50)
...
data = data.frame(treatments,weight,row,col)
data_x.hat = data.frame(treatments,weight_x.hat,row,col)
yio.bar=aggregate(weight_x.hat~as.factor(treatments),data=data,mean)$weight_x;yio.bar
...
for(i in 1:(len-1))
  {
    for(j in (i+1):len)
      {
  t_obs[k] = (yio.bar[i] - yio.bar[j])/sqrt(var)
  t_1[k] = i 
  t_2[k] = j
  k=k+1
    }
...
t_obs
qt(.975,15)
decision=ifelse(abs(t_obs)>qt(0.975,11),"Reject","Accept")
